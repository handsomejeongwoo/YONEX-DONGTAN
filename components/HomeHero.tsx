...
export default function HomeHero() {
  ...
  const logoOpacity = useTransform(scrollYProgress, [0.56, 0.67, 0.91, 1], [0, 1, 1, 0]);
  const logoScale = useTransform(scrollYProgress, [0.56, 0.71, 1], [0.72, 1, 18]);
  const logoY = useTransform(scrollYProgress, [0.72, 1], ["0vh", "-5vh"]);
  const introOpacity = useTransform(scrollYProgress, [0.965, 1], [1, 0]);
  ...
  return (
    <div ref={sceneRef} className="home-intro-scroll">
      ...
      >
        ...
        >
          {/* 실제 헤더 로고와 같은 원본 에셋. 새 심볼을 그려 넣지 않는다. */}
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src="/assets/yonex-dongtan-logo.png" alt="" />
        </motion.div>
